...
### Демонстрационный доступ в панель

`demo` / `demo`. Логин без домена работает: короткое имя дополняется до
`demo@globalex.uz` на сервере. Подсказка с этими данными показывается на
странице входа и на витрине **только при `SHOWCASE_ROOT=true`** — на сайте
компании такого блока не существует.
...
Перед передачей сайта компании демо-доступ нужно убрать:

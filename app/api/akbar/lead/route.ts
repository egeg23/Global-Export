...
/**
 * Приём заявок сайта Akbar Rich — расчёт, консультация, дилерство.
 *
 * Отдельный обработчик, а не общий с соседями: у фабрики свои поля — тема
 * заявки и набор из конструктора, — и всё это должно удаляться вместе с
 * папкой /akbar, когда проект отыграет.
 */
...
async function sendToTelegram(text: string): Promise<boolean> {
  ...
  const chatId = process.env.AKBAR_TELEGRAM_CHAT_ID || process.env.TELEGRAM_CHAT_ID;
  ...
  if (!response.ok) {
    console.error("Akbar: Telegram отказал", response.status);
    return false;
  }
  return true;
}
...
export async function POST(request: Request) {
  ...
  const rows: [string, string][] = [
    ["Имя", name],
    ["Телефон", phone],
    ["Тема", clean(lead.topic, 60)],
    ["Модель и размеры", clean(lead.model, 1200)],
    ["Страница", clean(lead.page, 200)],
  ];

  const text = [
    "<b>Заявка · Akbar Rich</b>",
    "",
    ...rows
  ...
  try {
    ...
    console.error("Akbar: заявка не ушла", error);
  }

  if (!delivered) console.info("Akbar: заявка без канала доставки\n" + text);

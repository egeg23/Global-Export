...
/**
 * Приём заявок сайта Delta IT-School — запись на пробный урок.
 *
 * Отдельный обработчик, а не общий с соседями: у школы свои поля — возраст
 * ребёнка и формат, — и всё это должно удаляться вместе с папкой /delta,
 * когда проект отыграет.
 */
...
async function sendToTelegram(text: string): Promise<boolean> {
  ...
  const chatId = process.env.DELTA_TELEGRAM_CHAT_ID || process.env.TELEGRAM_CHAT_ID;
  ...
  if (!response.ok) {
    console.error("Delta: Telegram отказал", response.status);
    return false;
  }
  return true;
}
...
export async function POST(request: Request) {
  ...
  const rows: [string, string][] = [
    ["Имя", name],
    ["Телефон", phone],
    ["Возраст ребёнка", clean(lead.age, 20)],
    ["Формат", clean(lead.format, 20)],
    ["Комментарий", clean(lead.note, 1200)],
    ["Страница", clean(lead.page, 200)],
  ];

  const text = [
    "<b>Запись на пробный урок · Delta IT-School</b>",
    "",
    ...rows
  ...
  try {
    ...
    console.error("Delta: заявка не ушла", error);
  }

  if (!delivered) console.info("Delta: заявка без канала доставки\n" + text);

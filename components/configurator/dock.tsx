 ...
 * та же на белом «Стандарте», бумажном «Люксе» и тёмном «Премиуме» — и на
...
export function Dock() {
  ...
  if (!ctx.open) {
    return (
      <button
        ...
        aria-expanded={false}
        className={cn(
          "w-dock-in fixed bottom-5 right-4 z-[70] items-center gap-3 rounded-full bg-[#0b0d10] font-sans py-3 pl-4 pr-5 text-sm text-[#f2efe9] shadow-2xl ring-1 ring-white/10 transition-transform duration-200 hover:-translate-y-0.5 motion-reduce:transform-none sm:right-5",
  ...
  const order = groupOrder(ctx.page, catalog.pages.map((page) => page.id), catalog.everywhere, (id) =>
    pageOf(catalog, id)?.shared !== false,
  );

  const copy = async () => {
    // Ссылка несёт набор всегда — даже если его читали из хранилища.
    write(catalog, ctx.tier, ctx.enabled);
    try {
      await navigator.clipboard.writeText(shareUrl(catalog, ctx.enabled));
      setCopied(true);
    } catch {
      window.prompt("Скопируйте ссылку", shareUrl(catalog, ctx.enabled));
    }
  };
  ...
  return (
    <aside
      ...
      className="w-dock-in fixed inset-x-0 bottom-0 z-[70] flex max-h-[64svh] flex-col rounded-t-2xl bg-[#0b0d10] font-sans text-[#f2efe9] shadow-2xl ring-1 ring-white/10 sm:inset-x-auto sm:bottom-5 sm:right-5 sm:w-[25rem] sm:max-h-[min(82vh,46rem)] sm:rounded-2xl"
    >
        ...
        <>
          ...
          <footer className="border-t border-white/10 px-5 py-4">
            ...
            <button
              ...
              className="mt-3 w-full rounded-full bg-[#ffd166] px-4 py-2.5 text-sm font-medium text-[#0b0d10] transition-opacity hover:opacity-90"
            >
              Отправить бриф
            </button>
            <div className="mt-2 flex gap-2">
              <button
                type="button"
                onClick={copy}
                className="flex-1 rounded-full border border-white/15 px-4 py-2.5 text-sm text-[#f2efe9]/80 transition-colors hover:border-white/40 hover:text-[#f2efe9]"
              >
...
/**
 * Порядок групп: эта страница, затем «везде», затем остальные. С панели
 * управления (не общей) «везде» встаёт после первой общей страницы — туда
 * тумблер и переведёт.
 */
export function groupOrder(
  page: string,
  pages: string[],
  everywhere: string | undefined,
  shared: (id: string) => boolean,
): string[] {
  const order = [page];
  if (everywhere && shared(page)) order.push(everywhere);
  for (const id of pages) {
    if (id === page) continue;
    order.push(id);
    if (everywhere && !order.includes(everywhere) && shared(id)) order.push(everywhere);
  }
  if (everywhere && !order.includes(everywhere)) order.push(everywhere);
  return order;
}

function Group({ where }: { where: string }) {
  const ctx = useConfigurator();
  if (!ctx) return null;

  const items = ctx.catalog.addons

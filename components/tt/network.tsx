    ...
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,0.5fr)] lg:gap-12">
      ...
      <div>
          ...
          {current ? (
            ...
            <>
              ...
              <p className="mt-3 text-[0.9rem] leading-relaxed text-[var(--w-muted)]">
                {t({
                  ru: "километров волоконно-оптических линий во всех регионах РК — их собственная цифра. Наведите на узел, чтобы увидеть плечи.",
                  kk: "ҚР барлық аймағындағы талшықты-оптикалық желі километрі — олардың өз саны. Иықтарды көру үшін түйінге меңзеңіз.",
                })}
              </p>
            </>
          )}
        ...
        <p className="mt-4 text-[0.78rem] leading-relaxed text-[var(--w-muted)]">
          {t({
            ru: `Схема показывает ${links.length} плеч между филиалами общей длиной ${shownKm.toLocaleString("ru-RU")} км: это характер сети, а не её полная топология. Волокно «Транстелекома» шло вдоль железной дороги — компания вышла из состава КТЖ.`,
            kk: `Схемада филиалдар арасындағы ${links.length} иық көрсетілген, жалпы ұзындығы ${shownKm.toLocaleString("ru-RU")} км: бұл желінің сипаты, толық топологиясы емес. «Транстелеком» талшығы теміржол бойымен жүрді — компания ҚТЖ құрамынан шыққан.`,
          })}
        </p>
      </div>
    </div>
